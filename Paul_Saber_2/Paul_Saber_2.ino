...
// ---------------------------- SETTINGS -------------------------------
#define NUM_LEDS 128         // number of microcircuits WS28xx on LED strip (note: one WS2811 controls 3 LEDs!)
#define BTN_TIMEOUT 500     // button hold delay, ms
#define BRIGHTNESS 50      // max LED brightness (0 - 255) max 44 for WS2812b 144 
#define LIGHT_UP_TIME 0.1 //ms per LED on strip
...
#define DEFAULT_COLOR 6     //0= red, 1= green, 5 = ice blue
    // 0 - red, 1 - green, 2 - blue, 3 - pink, 4 - yellow, 5 - ice blue, 6 = white
...
#define R2D2_WAV 34
#define POWER_OF_THE_DARK_SIDE_WAV 36
#define STRIKE_ME_DOWN_WAV 37
#define SOON_THE_REBELLION_WILL_BE_CRUSHED_WAV 38
#define YOUNG_FOOL_WAV 39
#define YOUR_FEEBLE_SKILLS_WAV 40
...
void setup() {
  ...
  if ((EEPROM.read(0) >= 0) && (EEPROM.read(0) <= 7)) {  // check first start
...
void btnTick() {
  ...
  if ((millis() - btn_timer > BTN_TIMEOUT) && (btn_counter != 0)) {
    if (lightsaber_on) {
      ...
      else if (btn_counter > 8) {               // 5 press count
      ...
      else if (btn_counter > 6) {     
        myDFPlayer.play(YOUNG_FOOL_WAV);
        humTimer = millis() - 9000 + 8500;  //This line is for turning the hum back on.  
      }
      else if (btn_counter > 5) {     
        myDFPlayer.play(YOUR_FEEBLE_SKILLS_WAV);
        humTimer = millis() - 9000 + 5500;  //This line is for turning the hum back on.  
      }
      else if (btn_counter > 4) {     
        myDFPlayer.play(POWER_OF_THE_DARK_SIDE_WAV);
        humTimer = millis() - 9000 + 4500;  //This line is for turning the hum back on.  
      }
      else if (btn_counter > 3) {     
        myDFPlayer.play(49);//Jabba laughing
        humTimer = millis() - 9000 + 5000;  //This line is for turning the hum back on.  
      }
      else if (btn_counter > 2) {               // 3 press count
        nowColor++;                         // change color
        if (nowColor >= 7) nowColor = 0;
        setColor(nowColor);
    ...
    //Else lightsaber off
    else if (!lightsaber_on)
    {
      if(btn_counter > 9) {   
        Serial.println(F("Playing sound effect..."));
        myDFPlayer.play(50);
        delay(10);
      }
      else if(btn_counter > 8) {   
        Serial.println(F("Playing sound effect..."));
        myDFPlayer.play(47);
        delay(10);
      }
        ...
        Serial.println(F("Playing sound effect..."));
        myDFPlayer.play(39);// Young fool...end        
        delay(10);
      }
      else if(btn_counter > 3) {     
        Serial.println(F("Playing sound effect..."));
        ...
        Serial.println(F("Playing sound effect..."));
        myDFPlayer.play(46);//Blaster
        delay(10);
      }
      if(btn_counter > 0) {   
        Serial.println(F("Playing sound effect..."));
        myDFPlayer.play(36);//Dark side
        delay(100);
      }
    }
    btn_counter = 0;
